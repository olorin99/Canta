...
void canta::ImGuiContext::render(ImDrawData *drawData, CommandHandle commandBuffer, Format format) {
    ...
    if (drawData->TotalVtxCount > 0) {
        ...
        if (!_vertexBuffer || _vertexBuffer->size() < vertexSize) {
            _vertexBuffer = _device->createBuffer({.size = static_cast<u32>(vertexSize * 2),
        ...
        if (!_indexBuffer || _indexBuffer->size() < indexSize) {
            _indexBuffer = _device->createBuffer({.size = static_cast<u32>(indexSize * 2),
    ...
    for (i32 i = 0; i < drawData->CmdListsCount; i++) {
        ...
        for (i32 j = 0; j < drawList->CmdBuffer.Size; j++) {
            ...
            if (cmd->UserCallback != nullptr) {
                ...
                commandBuffer->pushConstants(ShaderStage::FRAGMENT, Push{.samplerIndex = _sampler.index(), .sampledIndex = (i32)(u64)cmd->TextureId}, sizeof(f32) * 4);
...
bool canta::ImGuiContext::createFontsTexture(CommandHandle commandBuffer) {
    ...
    // Create the Image:
    _fontImage = _device->createImage({.width = static_cast<u32>(width),
                                       .height = static_cast<u32>(height),
                                       .format = Format::RGBA8_UNORM,
                                       .usage = ImageUsage::SAMPLED | ImageUsage::TRANSFER_DST});

    // Create the Upload Buffer:
    _uploadBuffer = _device->createBuffer({.size = static_cast<u32>(upload_size),
    ...
    commandBuffer->copyBufferToImage({.buffer = _uploadBuffer,
                                      .image = _fontImage,
                                      .dstLayout = ImageLayout::TRANSFER_DST});
    copyBarrier = {
        .image = _fontImage,
...
auto canta::ImGuiContext::createPipeline(canta::Format format) -> PipelineHandle {
    ...
    return _device->createPipeline({.vertex = {
                                        .spirv = std::vector(imgui_spv_embedded.begin(), imgui_spv_embedded.end()),
                                        .entry = "vertex"},
                                    .fragment = {.spirv = std::vector(imgui_spv_embedded.begin(), imgui_spv_embedded.end()), .entry = "fragment"},
                                    .rasterState = {.cullMode = CullMode::NONE, .frontFace = FrontFace::CCW, .polygonMode = PolygonMode::FILL, .lineWidth = 1.f},
                                    .blendState = {.blend = true, .srcFactor = BlendFactor::SRC_ALPHA, .dstFactor = BlendFactor::ONE_MINUS_SRC_ALPHA},
