...
auto canta::prefixSumExclusive(RenderGraph &graph, BufferIndex values, const u32 count) -> BufferIndex {
    ...
    const auto threadBlockReductions = graph.addBuffer({.size = static_cast<u32>(threadBlockReductionCount * sizeof(u32)),
                                                        .name = "thread_block_reduction"});

    const auto scanBump = graph.addBuffer({.size = sizeof(u32),
                                           .name = "scan_bump"});

    const auto sums = graph.addBuffer({.size = static_cast<u32>(count * sizeof(u32)),
                                       .name = "sums"});
