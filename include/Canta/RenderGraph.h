...
class RenderPass : public ende::graph::Vertex<BufferIndex, ImageIndex> {
  public:
    ...
    auto setManualPipeline(const bool state) -> RenderPass & {
        _manualPipeline = state;
        return *this;
    }
...
class PassBuilder {
  public:
    ...
    auto output(const u32 index = 0) -> std::expected<T, RenderGraphError> {
        if (_error)
            return std::unexpected(_error.value());
...
class RenderGraph : public ende::graph::Graph<RenderPass> {
  public:
    ...
    auto timers() -> std::span<TimerInfo> {
        if (_timers[_device->flyingIndex()].size() < _timerCount)
            return {};
    ...
    auto statistics() -> std::span<StatisticInfo> {
        if (_statistics[_device->flyingIndex()].size() < _statsCount)
            return {};
